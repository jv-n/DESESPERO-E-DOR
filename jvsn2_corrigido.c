...
void ordena (Aluno* alunos, int qtdAlunos)
{
    ...
    do
    {
        ...
        for (int i = 1; i<qtdAlunos; i++) //roda por todo o vetor de alunos
        {
            ...
            if(alunos[i].media==alunos[i-1].media) // se as medias forem iguais ele ordena alfabeticamente
            {
                if(strcmp(alunos[i].nome, alunos[i-1].nome)<0) //checa algm fora da ordem alfabetica
                {
                    troca = 1; //troca é verdadeira
                    char nometemp[50]; //variavel temporaria que vai armazenar o valor fora de ordem
                    strcpy(nometemp, alunos[i-1].nome);
                    strcpy(alunos[i-1].nome, alunos[i].nome);
                    strcpy(alunos[i].nome, nometemp);                
                }
            }
        }
